function [s_in t_in eff pmat]=M6(x,M,Y,Cov,r)
...
if isempty(r)
    warning('Missing User Threshold, defualt value will be used');
    r=0.1*(-log(0.02))+0.2*(-log(0.05))+0.5*(-log(0.1));
else
    r = -log(r);
end
if isempty(Cov)
    warning('No confounder adjusted');
    Cov = [];
end
...
if isempty(s_in)&&isempty(t_in)
    try
    [s_rev,t_rev, c_rev]=greedy_bipar(W,c_vec,1.8);
    s_in = setdiff(1:size(W,1),s_rev);
    t_in = setdiff(1:size(W,2),t_rev);
    catch ERM
    %fprintf('%s\n',ERM.message);
